...
static void OLED_PORT_ENABLE(void)
{
	__HAL_RCC_GPIOB_CLK_ENABLE();
}
static MyI2C_TypeStructure OLED_i2c_handle = {
	.SCL_Pin = GPIO_PIN_8,
	.SDA_Pin = GPIO_PIN_9,
	.SCL_GPIO_Port = GPIOB,
	.SDA_GPIO_Port = GPIOB,
	.GPIO_Clock_Enable = OLED_PORT_ENABLE,
};
/**
 * 数据存储格式：
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(OLED_i2c_handle.SCL_GPIO_Port, OLED_i2c_handle.SCL_Pin, (GPIO_PinState)BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	// HAL_GPIO_WritePin(OLED_PORT, OLED_SDA_PIN, (GPIO_PinState)BitValue);
	HAL_GPIO_WritePin(OLED_i2c_handle.SDA_GPIO_Port, OLED_i2c_handle.SDA_Pin, (GPIO_PinState)BitValue);
...
/*
// FIXME: 浮点数无法正常显示，临时替代方法如下
	float f1 = 1.23456789;
	int int_part = (int)(f1);
	int dec_part = (int)(f1 * 100) % 100;
	OLED_Printf(0, 16, OLED_8X16, "float:%4d.%2d", int_part, dec_part);
*/
